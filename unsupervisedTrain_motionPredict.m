...
function [] = unsupervisedTrain_motionPredict(X_input, total_columns, total_epochs, save_freq, folderSave, fname_AEsave)
...
%             selfOrgNet(num_layer).S = sparse(zeros(selfOrgNet(num_layer).numNeurons));
    ...
    for ss = 1:total_epochs
        ...
        num_futureFrames = 50;
        Tend = size(X_input,2)-num_futureFrames; dt = 0.1; t_intrvls = 0:Tend/dt;  tx = 0; Trot = 1*Tend; ts = 0;  
        ...
        for tt = 0:dt:Tend-dt
            ...
            if mod(tt,1) == 0
                ...
                tx = tx+1;
                Xtmp = X_input(:,tx); 
                X_reconstruct  = sparse(X_input(:,tx+num_futureFrames));
                ...
                %fireRec = find(X_reconstruct >= selfOrgNet(num_layer).th);
                
                selfOrgNet(total_layers).Hdes = X_reconstruct;
