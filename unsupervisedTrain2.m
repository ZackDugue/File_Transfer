...
function [] = unsupervisedTrain2(X_input, total_columns, total_epochs, save_freq, folderSave, fname_AEsave,num_layers)

%% DEFINING HYPERPARAMETERS OF THE NETWORK

opt = {};
opt.total_layers = num_layers;

opt.nTwin = [];

nTwin_L1 = normrnd(100, 50, [1,total_columns]);
...
for ii = 2:opt.total_layers-1
    nTwin_col = normrnd(100, 50, [1,total_columns]);
    nTwin_col(nTwin_col<1) = 1;   
    nTwin_col = floor(nTwin_col);
    %nTwin_col(1:end) = 1;
    opt.nTwin = [opt.nTwin; nTwin_col];

    rndNumNeurons = floor(normrnd(250,50,[1, total_columns]));
    rndNumNeurons(rndNumNeurons<1) = 1;
...
opt.tau_v = [ones(1, total_columns); 1*ones(1, total_columns); ones(1, total_columns); ones(1, total_columns); ones(1, total_columns);  ones(1, total_columns);  ones(1, total_columns); 1*ones(1, total_columns); 1*ones(1, total_columns); 1*ones(1, total_columns); 1*ones(1, total_columns); 1*ones(1, total_columns); 1*ones(1, total_columns)];
opt.tau_th = [ones(1, total_columns); 2*ones(1, total_columns); 1*ones(1, total_columns); ones(1, total_columns); ones(1, total_columns); ones(1, total_columns); ones(1, total_columns); 1*ones(1, total_columns); 1*ones(1, total_columns); 1*ones(1, total_columns); 1*ones(1, total_columns); 1*ones(1, total_columns)];
opt.th_plus = [ones(1, total_columns); 2*ones(1, total_columns); 1*ones(1, total_columns); ones(1, total_columns); ones(1, total_columns); ones(1, total_columns); ones(1, total_columns); 1*ones(1, total_columns); 1*ones(1, total_columns); 1*ones(1, total_columns); 1*ones(1, total_columns); 1*ones(1, total_columns)];
opt.v_th = [ones(1, total_columns); 5*ones(1, total_columns); 5/2*ones(1, total_columns); 5/3*ones(1, total_columns);ones(1, total_columns); 5/4*ones(1, total_columns); 5/5*ones(1, total_columns); ones(1, total_columns); 1*ones(1, total_columns); 1*ones(1, total_columns); 1*ones(1, total_columns); 1*ones(1, total_columns); 1*ones(1, total_columns)];

opt.kWTA = [ones(1,total_columns); 50*ones(1, total_columns); 50*ones(1, total_columns); 50*ones(1,total_columns); 50*ones(1, total_columns); 50*ones(1, total_columns); 50*ones(1,total_columns);50*ones(1,total_columns);50*ones(1,total_columns);50*ones(1,total_columns);50*ones(1,total_columns)];

tot_epochs = [];

%opt.nTwin(:,1) = ones(opt.total_layers,1);
% opt.numNeurons
% opt.nTwin
...
for colId = 1:total_columns
    ...
    dim_layers(4).s = [10,10];
    dim_layers(5).s = [80,101];
    dim_layers(6).s = [10,101];
    dim_layers(7).s = [80,101];
    dim_layers(8).s = [10,101];
    dim_layers(9).s = [80,101];
    dim_layers(10).s = [10,101];
    dim_layers(11).s = [80,101];
    dim_layers(12).s = [10,101];
    dim_layers(13).s = [80,101];
    dim_layers(14).s = [10,101];
    dim_layers(15).s = [80,101];
    dim_layers(total_layers).s = [80,101]
    % CREATING 2+ LAYERS
    ...
    J_vec = []; lr_vec = []; lr = 10; conv_ctr = 0; minJ = 0.1; spdp = 1; %spdp=speedup factor
    ...
    for ss = 1:total_epochs
        ...
        for tt = 0:dt:Tend-dt
            ...
            if mod(tt,1) == 0
                ...
                if mod(tt,Trot)==0
                    if state_reset == true
                        %disp(sprintf('Entering state-reset at %d',tt));
...
%     
    ...
    % 	save(fname_save, 'selfOrgNet','netLossVal')
        ...
        if mod(ss, save_freq) == 0
            fname_save2 = strcat(fname_AEsave, sprintf('column=%d_ep=%d',colId,ss));
